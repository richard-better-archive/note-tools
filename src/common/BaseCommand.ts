...
export abstract class BaseCommand
  extends Command<CommandContext>
  implements BaseCommandProps {
  /**
   ...
   */
  public static Path(command: keyof NoteTools.Commands) {
   ...
   */
  public async run<Cmd extends keyof NoteTools.Commands>(
    command: Cmd,
    { positional = [], ...other }: RunSubCommandParams<NoteTools.Commands[Cmd]>
  ) {
    const _ = [...(command ? command.split(" ") : []), ...positional];
    const args = dargs(
      { _, verbose: this.verbose, cwd: this.cwd, ...other },
      {}
    );
...
export type GetShapeOfCommandData<Cmd extends BaseCommand> = ConditionalPick<
  Cmd,
  FlaggedCommand
>;

export type RunSubCommandParams<
  CommandData extends GetShapeOfCommandData<BaseCommand>
> = {
  positional?: string[];
} & Partial<CommandData>;
